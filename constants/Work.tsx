export const WORKS = [
  {
    id: 1,
    title: "Horrorween Haunted Warehouse",
    description:
      "This website was created for the Horrorween Haunted Warehouse.  The website is a spooky, simple, and easy to use website that is designed to be used on any device.  I created it using SquareSpace for ease of use for their staff.  Their website is subject to change as they update their content.",
    image: "/horrorween.png",
    link: "https://www.horrorweenhw.com/",
    button: "View Project",
  },
  {
    id: 2,
    title: "Maxx Powersports LLC",
    description:
      "This website was created for Maxx Powersports LLC.  The website is a modern, clean, and easy to use website that is designed to be used on any device.  I created it using SquareSpace for ease of use for the business staff.  Their website is subject to change as they update their content.",
    image: "/maxx.png",
    link: "https://www.maxx-powersports.com/",
    button: "View Project",
  },
  {
    id: 3,
    title: "Orbisonia United Methodist Church",
    description:
      "This website was created for Orbisonia United Methodist Church.  The website is a simple, clean, and easy to use website that is designed to be used on any device.  I created it using SquareSpace for ease of use for the church staff.  Their website is subject to change as they update their content.",
    image: "/orbyumc.png",
    link: "https://www.orbyumc.com/",
    button: "View Project",
  },
  {
    id: 4,
    title: "Woody's Beef Jerky",
    description:
      "This website was created for Woody's Beef Jerky.  Because of this website, Woody's Beef Jerky has been able to expand their reach and sell their jerky all across the United States.  I created this website using SquareSpace for ease of use for the business owner.  The website is subject to change as they update their content.",
    image: "/woodysbeefjerky.png",
    link: "https://www.woodysbeefjerky.com/",
    button: "View Project",
  },
  {
    id: 5,
    title: "Mifflin County Youth Fair",
    description:
      "This website was created for Mifflin County Youth Fair.  Thanks to this website, the Mifflin County Youth fair has been able to expand their reach and get relevant information to their all of their fans.  I created it using SquareSpace for ease of use for the fair staff.  This website has been updated with some new content since it was designed, but overall the design is still the same. The website is still subject to change as they update their content further.",
    image: "/mcyouthfair.png",
    link: "https://www.mcyouthfair.org/",
    button: "View Project",
  },
  {
    id: 6,
    title: "Our Etsy Store",
    description:
      "Prough Web Design, LLC crafts elegant and user-friendly SquareSpace templates, tailored to elevate your online presence. We also offer a variety of custom designs available for purchase on our Etsy store.",
    image: "/Etsy_logo.png",
    link: "https://www.etsy.com/shop/ProughWebDesignLLC",
    button: "View Store",
  },
  {
    id: 7,
    title: "Github Repositories",
    description:
      "Please check out my Github repositories to see the code for my projects, including this website, which was created using Next.js and Tailwind CSS.  I have a variety of projects that I have worked on, and I am always looking to improve my skills and learn new things.",
    image: "/github-mark-white.png",
    link: "https://github.com/nprough",
    button: "View Profile",
  },
];
